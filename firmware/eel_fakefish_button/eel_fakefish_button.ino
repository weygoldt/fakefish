...
static void onSampleTick() {
  ...
  if (sdplayer.ring.pop(&s)) {
    ...
    sampleClock.end();
    out_silence();   // brake BOTH bridges to GND and clear the shaper (never leave duty asserted)
...
static void start_playback(int b) {
  ...
  out_silence();   // clear the noise-shaper so stale error can't glitch the first samples
