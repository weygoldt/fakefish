// Operator interface for the SD-card fakefish: SIX program buttons + an indicator LED.
//
...
// lists that directory, picks a WAV at random, and streams it (see sd_player.h). All the
// on-device marker / session / overlap-add machinery of the old flash-library firmware is
// gone — a stimulus is now simply a pre-rendered WAV on the card (built by the offline
// Python tool src/fakefish/build_sd_card.py).
//
// The pure logic (debounce edge-detect, button->directory map) sits ABOVE the
// `#ifdef ARDUINO` line so it compiles and is unit-tested on a PC
// (firmware/eel_fakefish/host_test/eel_control_selftest). Only the GPIO / millis() glue is
// Arduino-specific.
#pragma once
#include <stdint.h>
...
// runs is IGNORED (playbacks are uninterruptible). Pins 5-10 avoid the PWM output pins
// (2, 3) and A0 (== digital 14, randomSeed); the 4.1's SDIO card slot is on dedicated pads,
// so it does not collide. Verify 9/10 are free on your board.
...
static inline int eel_control_button_fired() {
